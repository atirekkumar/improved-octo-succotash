    ...
    rules: [
      ...
      },
      {
        ...
        exclude: [
          ...
          /improved-octo-succotash.*\/node_modules\//,
          /\.test.jsx?$/,
        ],
        include: [
          new RegExp(`${APP_DIR}/src`),
          /superset-ui.*\/src/,
          /improved-octo-succotash.*\/src/,
      ...
      {
        test: /\.css$/,
        include: [APP_DIR, /superset-ui.+\/src/, /improved-octo-succotash.+\/src/],
        use: [
...
if (isDevMode) {
  ...
  Object.entries(packageConfig.dependencies).forEach(([pkg, version]) => {
    ...
    if (
      (/superset-ui/.test(pkg) || /improved-octo-succotash/.test(pkg)) &&
